...
def connect_to_server():
    ...
    print(f"[{get_timestamp()}] Connection between client {sock.getsockname()} and server {server_address} is established")
...
def browse_and_play_canned_audio():
    file_path = fd.askopenfilename(initialdir="/CannedRecordings", title="Select a WAV File", filetypes=(("WAV Files", "*.wav"), ("All Files", "*.*")))
    if file_path:
...
button_browse_audio = Button(client_window, text="Browse and Send Audio Recording", command=browse_and_send_audio)
button_browse_audio.pack()

# GUI button to browse and send text files
button_browse_text = Button(client_window, text="Browse and Send Text File", command=browse_and_send_text_file)
button_browse_text.pack()

# GUI button to browse and play canned audio recordings
button_play_canned_audio = Button(client_window, text="Browse and Play Canned Audio", command=browse_and_play_canned_audio)
